...
- `.vscodeignore` - specifies files and folders to exclude from the extension package.

## Get up and running straight away

- Press `F5` to open a new window with your extension loaded.
- Open the color theme picker with the `File > Preferences > Theme > Color Theme` menu item, or use the `Preferences: Color Theme` command (`Ctrl+K Ctrl+T` / `Cmd+K Cmd+T`) and pick **Nexus Dark (Beta)**.
